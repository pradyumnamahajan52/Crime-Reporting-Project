...
const Profile = () => {
  ...
  const handleSubmit = async (e, userInfo) => {
    ...
    try {
      ...
      navigate("/police/profile?success=Profile updated successfully!");
    } catch (error) {
...
const ProfileSidebar = ({ fullName, role }) => (
  <>
    ...
    />
    <h3 className="text-xl font-semibold text-gray-700">{fullName || "Guest Police"}</h3>
    <p className="text-sm text-gray-500">{role || "Police"}</p>
  </>
);
